...
resource "null_resource" "ansible-provisioning" {
  ...
  depends_on = [
    azurerm_windows_virtual_machine.eh-ctf-vm-dc,
    azurerm_windows_virtual_machine.eh-ctf-vm-winserv2019,
    azurerm_windows_virtual_machine.eh-ctf-vm-windows10,
    azurerm_linux_virtual_machine.eh-ctf-vm-hackbox,
  ]
  ...
  connection {
    type  = "ssh"
    host  = azurerm_public_ip.eh-ctf-ip.ip_address
    user  = var.linux-user
    password = random_string.linuxpass.result
  }
